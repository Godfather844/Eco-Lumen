...
import { 
  ...
  HelpCircle, CheckCircle, TrendingDown, Trees
} from 'lucide-react';
...
export default function Home({ setCurrentScreen }: HomeProps) {
  ...
  // Dynamic calculations for the simulator wrapped in useMemo for optimal rendering performance
  const metrics = useMemo(() => {
    const baselineFootprint = 18.4; // Metric tons per user or million tons per company
    const simulatedFootprint = Math.max(
      2.1, 
      Number((baselineFootprint * (1 - effortLevel / 100)).toFixed(1))
    );
    const simulatedOffset = Math.min(
      10.0, 
      Number((effortLevel * 0.1).toFixed(1))
    );
    const targetYear = Math.max(2028, 2050 - Math.floor(effortLevel * 0.22));

    const decreasePct = Math.round(((baselineFootprint - simulatedFootprint) / baselineFootprint) * 100);
  ...
  // Handle keyboard modification for the effort slider to meet WCAG standards
  const handleSliderKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'ArrowRight' || e.key === 'ArrowUp') {
      setEffortLevel((prev) => Math.min(100, prev + 5));
  ...
  return (
    <article id="home-screen" className="flex-1 flex flex-col bg-zinc-950 text-zinc-100 selection:bg-indigo-600 selection:text-white pb-16 font-sans">
      
      {/* SECTION 1: HERO SECTION */}
      <section 
        id="hero-banner" 
        className="relative overflow-hidden pt-12 md:pt-16 pb-20 border-b border-zinc-900"
        aria-labelledby="hero-title"
      >
        <div id="hero-dot-pattern" className="absolute inset-0 opacity-15 bg-[radial-gradient(#6366f1_1px,transparent_1px)] [background-size:16px_16px]" aria-hidden="true"></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            >
              <div id="hero-badge" className="inline-flex self-start items-center space-x-2 bg-indigo-500/10 border border-indigo-500/20 py-1.5 px-3 rounded-full text-indigo-300 text-xs font-bold uppercase tracking-widest">
                <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 animate-pulse" aria-hidden="true"></span>
              ...
              <div id="hero-ctas" className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4 pt-4">
                <button
                  ...
                  aria-label="Begin carbon emission analysis"
                >
                ...
                <button
                  ...
                  className="bg-zinc-900 hover:bg-zinc-800 text-zinc-100 border border-zinc-800 hover:border-zinc-700 px-6 py-3.5 rounded-2xl text-xs font-extrabold font-display uppercase tracking-wider text-center transition-all duration-200 cursor-pointer focus:outline-none focus:ring-2 focus:ring-zinc-700 focus:ring-offset-2 focus:ring-offset-zinc-950"
                  aria-label="Get started with employee progress logger"
                >
              ...
              <div className="grid grid-cols-3 gap-6 border-t border-zinc-900 pt-8 mt-4 font-display" aria-label="Eco stats summaries">
                <div>
            ...
            >
              <div id="hero-photo-wrapper" className="relative group overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900 shadow-2xl">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-transparent to-transparent" aria-hidden="true"></div>
      ...
      {/* SECTION 2: THE SMARTER METHOD */}
      <section id="bento-presentation" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20" aria-labelledby="bento-heading">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section 
            id="bento-card-1"
            className="group relative bg-zinc-900/60 p-8 rounded-3xl border border-zinc-800/80 hover:border-indigo-500/40 hover:shadow-indigo-500/5 hover:shadow-2xl transition-all duration-300 flex flex-col justify-between overflow-hidden cursor-pointer"
            onClick={() => setCurrentScreen('log-progress')}
            tabIndex={0}
            onKeyDown={(e) => e.key === 'Enter' && setCurrentScreen('log-progress')}
            aria-label="Automatic Utility Synchronization method details"
          >
            <div id="bento-dec-1" className="absolute top-0 right-0 w-32 h-32 bg-indigo-505/5 blur-3xl rounded-full" aria-hidden="true"></div>
            <div>
          ...
          {/* Card 2: Compelling Analytics */}
          <section 
            id="bento-card-2"
            className="group relative bg-zinc-900/60 p-8 rounded-3xl border border-zinc-800/80 hover:border-purple-500/40 hover:shadow-purple-500/5 hover:shadow-2xl transition-all duration-300 flex flex-col justify-between overflow-hidden cursor-pointer"
            onClick={() => setCurrentScreen('dashboard')}
            tabIndex={0}
            onKeyDown={(e) => e.key === 'Enter' && setCurrentScreen('dashboard')}
            aria-label="Scope 1, 2, and 3 Greenhouse Gas Reporting details"
          >
            <div id="bento-dec-2" className="absolute top-0 right-0 w-32 h-32 bg-purple-500/5 blur-3xl rounded-full" aria-hidden="true"></div>
            <div>
          ...
          {/* Card 3: High Precision Audit */}
          <section 
            id="bento-card-3"
            className="group relative bg-zinc-900/60 p-8 rounded-3xl border border-zinc-800/80 hover:border-teal-500/40 hover:shadow-teal-500/5 hover:shadow-2xl transition-all duration-300 flex flex-col justify-between overflow-hidden cursor-pointer"
            onClick={() => setCurrentScreen('about')}
            tabIndex={0}
            onKeyDown={(e) => e.key === 'Enter' && setCurrentScreen('about')}
            aria-label="Third-Party Audit Validation protocols"
          >
            <div id="bento-dec-3" className="absolute top-0 right-0 w-32 h-32 bg-teal-500/5 blur-3xl rounded-full" aria-hidden="true"></div>
            <div>
          ...
          {/* Card 4: Forecast Simulator */}
          <section 
            id="bento-card-4"
            className="group relative bg-zinc-900/60 p-8 rounded-3xl border border-zinc-800/80 hover:border-amber-500/40 hover:shadow-amber-500/5 hover:shadow-2xl transition-all duration-300 flex flex-col justify-between overflow-hidden cursor-pointer"
            onClick={() => setCurrentScreen('calculator')}
            tabIndex={0}
            onKeyDown={(e) => e.key === 'Enter' && setCurrentScreen('calculator')}
            aria-label="Dynamic Offset Scenario Forecasting simulation parameters"
          >
            <div id="bento-dec-4" className="absolute top-0 right-0 w-32 h-32 bg-amber-500/5 blur-3xl rounded-full" aria-hidden="true"></div>
            <div>
      ...
      <section id="interactive-simulator-section" className="bg-zinc-950/40 py-16 border-y border-zinc-905 relative" aria-labelledby="simulator-heading">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <h2 id="simulator-heading" className="text-3xl font-extrabold text-white tracking-tight leading-tight font-display">
                Simulate Your Organizational Effort
              </h2>
              <p className="text-zinc-400 text-sm leading-relaxed">
                Adjust the sliding environmental effort gauge to approximate vertical reduction targets. View the real-time simulation engine calculating emissions reduction schedules, offset rates, and carbon-neutral milestones immediately.
              </p>
            ...
            {/* Simulator Right Play area */}
            <section className="lg:col-span-7 bg-zinc-900 border border-zinc-800 rounded-3xl p-6 md:p-8 flex flex-col justify-between shadow-2xl relative" aria-label="Interactive Organization Simulator widget">
              <div>
                ...
                <div id="effort-slider-group" className="space-y-4 mb-8">
                  ...
                  <div className="flex flex-col gap-2">
                    <label id="sim-effort-label" htmlFor="sim-effort-slider" className="sr-only">Organizational Carbon Reduction Effort Level</label>
                    <input
                      ...
                      max="100"
                      step="1"
                      aria-labelledby="sim-effort-label"
                      aria-valuemin={0}
                      aria-valuemax={100}
                      aria-valuenow={effortLevel}
                      value={effortLevel}
                      onKeyDown={handleSliderKeyDown}
                      onChange={(e) => setEffortLevel(safeParsePositiveNumber(e.target.value, 0, 100))}
                      className="w-full h-2.5 bg-zinc-950 rounded-lg appearance-none cursor-pointer accent-indigo-600 outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 bg-zinc-950 p-5 rounded-2xl border border-zinc-800/80" aria-label="Simulated environment predictions outputs">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-baseline space-x-1">
                      <span className="text-3xl font-extrabold text-white font-mono">{metrics.simulatedFootprint}</span>
                      <span className="text-xs text-zinc-400"> Tons</span>
                    </div>
                    <span className="text-[11px] text-emerald-400 font-bold block">
                      -{metrics.decreasePct}% decrease
                    </span>
                  </div>
                  ...
                  <div className="border-t sm:border-t-0 sm:border-x border-zinc-900 pt-4 sm:pt-0 sm:px-6 space-y-1">
                    ...
                    <div className="flex items-baseline space-x-1">
                      <span className="text-3xl font-extrabold text-white font-mono">{metrics.simulatedOffset}</span>
                      <span className="text-xs text-zinc-400"> Tons</span>
                    </div>
                  ...
                  <div className="border-t sm:border-t-0 pt-4 sm:pt-0 space-y-1">
                    ...
                    <div className="flex items-baseline space-x-1">
                      <span className="text-3xl font-extrabold text-purple-400 font-mono">{metrics.targetYear}</span>
                    </div>
                    <span className="text-[11px] text-indigo-400 font-bold block">
                      {metrics.targetYear <= 2035 ? 'Aggressive Shift' : 'Standard Path'}
                    </span>
                  </div>
              ...
              <div className="mt-8 flex justify-end">
                <button
                  id="sim-action-btn"
                  onClick={() => setCurrentScreen('calculator')}
                  className="bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 border border-indigo-500/25 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer flex items-center space-x-2 font-display focus:outline-none focus:ring-2 focus:ring-indigo-550"
                  aria-label="Build a live carbon reduction plan using our calculator"
                >
      ...
      <section id="features-highlights" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20" aria-labelledby="highlights-heading">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center mb-16">
          <div className="lg:col-span-2 space-y-4">
            <h2 id="highlights-heading" className="text-3xl sm:text-4xl font-extrabold tracking-tight text-white leading-tight font-display">
          ...
          <div className="lg:col-span-1 lg:text-right">
            <button
              onClick={() => setCurrentScreen('log-progress')}
              className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3.5 rounded-2xl text-xs font-bold uppercase tracking-wider shadow-lg hover:shadow-indigo-500/20 cursor-pointer transition-colors focus:ring-2 focus:ring-indigo-500 focus:outline-none"
              aria-label="Sign up your team for our program"
            >
              Sign Up Your Team
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {/* Feature Card 1 */}
          <section className="bg-zinc-900/60 p-6 rounded-3xl border border-zinc-800 hover:border-zinc-700 transition-all flex flex-col justify-start" aria-labelledby="feat-label-1">
            ...
            <h4 id="feat-label-1" className="text-base font-bold text-white mb-2 font-display">Carbon Dashboard</h4>
          ...
          {/* Feature Card 2 */}
          <section className="bg-zinc-900/60 p-6 rounded-3xl border border-zinc-800 hover:border-zinc-700 transition-all flex flex-col justify-start" aria-labelledby="feat-label-2">
            ...
            <h4 id="feat-label-2" className="text-base font-bold text-white mb-2 font-display">Automated Compliance</h4>
            <p className="text-zinc-400 text-xs leading-relaxed">
          ...
          {/* Feature Card 3 */}
          <section className="bg-zinc-900/60 p-6 rounded-3xl border border-zinc-800 hover:border-zinc-700 transition-all flex flex-col justify-start" aria-labelledby="feat-label-3">
            ...
            <h4 id="feat-label-3" className="text-base font-bold text-white mb-2 font-display">Interactive habit log</h4>
            <p className="text-zinc-400 text-xs leading-relaxed">
          ...
          {/* Feature Card 4 */}
          <section className="bg-zinc-900/60 p-6 rounded-3xl border border-zinc-800 hover:border-zinc-700 transition-all flex flex-col justify-start" aria-labelledby="feat-label-4">
            ...
            <h4 id="feat-label-4" className="text-base font-bold text-white mb-2 font-display">Personal Eco Tree</h4>
            <p className="text-zinc-400 text-xs leading-relaxed">
      ...
      {/* SECTION 5: SECOND CALL TO ACTION BANNER */}
      <section id="hero-3-cta-banner" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" aria-label="Stewardship program invitation panel">
        <div className="relative overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-900/60 shadow-2xl">
          <img 
            ...
            alt="Wide majestic high contrast landscape representing clean energy future with mountains and sunrise" 
            className="absolute inset-0 w-full h-full object-cover opacity-20" 
          />
          <div className="absolute inset-0 bg-gradient-to-r from-zinc-950 via-zinc-950/80 to-transparent" aria-hidden="true"></div>
          ...
          <div className="relative z-10 px-8 py-16 sm:px-16 sm:py-24 max-w-2xl flex flex-col space-y-6">
            ...
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4 pt-4">
              <button
                onClick={() => setCurrentScreen('calculator')}
                className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-3.5 rounded-2xl text-xs font-bold uppercase tracking-wider shadow-lg hover:shadow-indigo-500/20 cursor-pointer transition-colors font-display focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                aria-label="Begin Your Carbon Analysis"
              >
                Begin Your Analysis
              </button>
              <button
                onClick={() => setCurrentScreen('log-progress')}
                className="bg-zinc-800 hover:bg-zinc-700 text-white border border-zinc-705 px-6 py-3.5 rounded-2xl text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer font-display focus:ring-2 focus:ring-zinc-750 focus:outline-none"
                aria-label="Join the movement by signing up"
              >
                Join the Movement
              </button>
            </div>
